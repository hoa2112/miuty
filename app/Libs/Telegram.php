...
class Telegram
{
    ...
    {
        $user = $this->getChatMember(array("chat_id" => $chat_id, "user_id" => $user_id));
        if($user['ok']){
            return $user['result']['status'] == 'administrator' || $user['result']['status'] == 'creator';
        }
        return false;
    }
    ...
    private function sendAPIRequest($url, array $content, $post = true)
    {
        ...
        try {
            ...
            if($response->getStatusCode() == 200){
                //ghi log
                ThirdPartyLog::log([
                    'action' => 'Telegram - Send Request API',
                    'url_request' => $url,
                    'params' => $content,
